...
pub fn scan_files(directory: String, build_settings: &BuildSettings) {
...
pub fn build_love(directory: String, build_settings: &BuildSettings) {
    let method = METHOD_DEFLATED;

    let src_dir = &directory;
    let dst_file: &str = "test.love";

    match collect_zip_directory(src_dir, dst_file, method.unwrap(), build_settings) {
        Ok(_) => {
